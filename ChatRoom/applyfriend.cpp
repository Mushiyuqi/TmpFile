    ...
    , ui(new Ui::ApplyFriend)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText(tr("牧师屿崎"));
    ui->lb_ed->setPlaceholderText("搜索、添加标签");
    ui->back_ed->setPlaceholderText("超级小能猫");
...
void ApplyFriend::AddTipLbs(ClickedLabel* lb, QPoint cur_point, QPoint &next_point,
...
void ApplyFriend::SlotLabelEnter()
{
    ...
    if (find_add != m_add_labels.end()) {
        find_add.value()->SetCurState(ClickState::Selected);
        return;
    }
    ...
    lb->SetCurState(ClickState::Selected);
...
void ApplyFriend::SlotChangeFriendLabelByTip(QString lbtext, ClickState state)
{
